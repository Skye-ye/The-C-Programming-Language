...
int dircmp(const char *, const char *, int);
  ...
  if ((nlines = readlines(lineptr, MAXLINES)) >= 0) {
    if (numeric) {
      quick_sort((void **)lineptr, 0, nlines - 1,
                 (int (*)(const void *, const void *))numcmp, reverse);
    } else if (directory) {
      quick_sort((void **)lineptr, 0, nlines - 1,
                 (int (*)(const void *, const void *))dircmp, reverse);
    } else {
      quick_sort((void **)lineptr, 0, nlines - 1,
                 (int (*)(const void *, const void *))(fold ? foldcmp : strcmp),
                 reverse);
    }
...
int dircmp(const char *s1, const char *s2, int fold) {
  do {
    while (*s1 && !isalnum((unsigned char)*s1) &&
           !isspace((unsigned char)*s1)) {
      s1++;
    }
    while (*s2 && !isalnum((unsigned char)*s2) &&
           !isspace((unsigned char)*s2)) {
      s2++;
    }
    int c1 = fold ? tolower((unsigned char)*s1) : (unsigned char)*s1;
    int c2 = fold ? tolower((unsigned char)*s2) : (unsigned char)*s2;
    if (c1 != c2) {
      return c1 - c2;
    }
  } while (*s1++ && *s2++);
